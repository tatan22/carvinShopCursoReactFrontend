...
import type { Product, Size } from "@/interfaces/products.interface";
...
export const ProductForm = ({
	...
	// La linea de abajo sobraría al implementar la interfaz
	...
	const addTag = () => {
		const newTags = labelInputRef.current?.value || "";
		if (!newTags) return;
		const newTagSet = new Set(getValues("tags"));
		newTagSet.add(newTags);
	...
	const addSize = (size: Size) => {
		...
		sizeSet.add(size);
		// Con Array from convertimos el Set en un array
	...
	//? Estas son las dos maneras con las que podemos hacer drag and drop
	...
	const handleDrop = (e: React.DragEvent) => {
		...
		if (!files) return; // Retorno anticipado
	...
	return (
		<form onSubmit={handleSubmit(onSubmit)}>
			<div className="flex justify-between items-center">
				<AdminTitle title={tittle} subtitle={subTitle} />
								...
								<div>
									...
									<input
										...
										{...register("slug", {
											...
											validate: (value) =>
												!/\s/.test(value) ||
												"El Slug no debe contener espacios",
										})}
										className={cn(
											"w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200",
											{
							...
							>
								...
								<div className="grid grid-cols-2 gap-3">
									{files.map((file, index) => (
										<img
											key={index} //? No es lo mas recomendado pero funciona
